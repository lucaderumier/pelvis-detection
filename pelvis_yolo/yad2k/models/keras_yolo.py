...
def yolo_loss(args,
    ...
    #object_detections = tf.Print(object_detections,[tf.shape(object_detections)[2:]],message = 'OBJECT DETECTION')
    ...
    # Shapes before sum :
    confidence_loss_sum = K.sum(confidence_loss) # (n_images,S,S,n_achors,1)
    classification_loss_sum = K.sum(classification_loss) # (n_images,S,S,n_achors,n_classes)
    coordinates_loss_sum = K.sum(coordinates_loss) # (n_images,S,S,n_achors,1)
...
def yolo_eval(yolo_outputs,
    ...
    # Hacking with type cast otherwise int32 and float32 did not match..
    #boxes = tf.dtypes.cast(boxes, tf.int32)
    image_dims = tf.dtypes.cast(image_dims, tf.float32)
...
class metric():
    def __init__(self,args,image_data_shape,anchors,num_classes,max_boxes=10,score_threshold=.6,iou_threshold=.5,rescore_confidence=False):
        self.args = args
        self.image_data_shape = image_data_shape
    ...
    def conf_loss(self,y_true,y_pred):
        ...
        return confidence_loss_sum

    def coord_loss(self,y_true,y_pred):
        '''Coordinates loss metric'''
        (yolo_output,true_boxes, detectors_mask, matching_true_boxes) = self.args
