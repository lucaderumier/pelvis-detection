...
MODE = 'train' #'train', 'test', 'try', 'test_on_train', 'test_on_val' or 'extract'
...
if(MODE == 'try'):
    ...
    DATASET_DIR = os.path.join('pelvis_scan','DATA_CT_SPARSE')
...
class Config():
    ...
    LOAD_PREVIOUS_MODEL = False # Loads a previously trained model (WEIGHTS)
    NON_BEST_SUP = True
    LEARNING_RATE = 0.001
    BATCH_SIZE = 32
...
def _main():
    ...
    if(MODE == 'extract'):
        ...
        # Ids
        list_ids_train = sorted([x for x in os.listdir(os.path.join(DATASET_DIR,'train')) if x.endswith('.jpg')])
        list_ids_val = sorted([x for x in os.listdir(os.path.join(DATASET_DIR,'val')) if x.endswith('.jpg')])


        # Data generators
        datagen_train = DataGenerator(list_ids_train, data_train, anchors, **params)
        datagen_val = DataGenerator(list_ids_val, data_val, anchors, **params)
        image_data, _ = process_data(data_train['images'], data_train['boxes'])
...
def train(model,training_generator,validation_generator,train_size,batch_size,data_shape,class_names, anchors,config,validation_split=0.1,weights_path = '',results_dir=''):
    '''
    ...
    for step in range(15):
        ...
        # Saving history
        with open(os.path.join(results_dir,'history','history'+str(step)+'.p'), 'wb') as fp:
            pickle.dump(history.history, fp)

        model.save_weights(os.path.join(results_dir,'models','trained_stage_'+str(step)+'.h5'))

        model.load_weights(os.path.join(results_dir,'models','trained_stage_'+str(step)+'.h5'))
